...
type Step = "choose" | "token" | "validating" | "branches" | "confirmed" | "error";
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg mx-4 rounded-2xl bg-card border border-border shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        {/* Step indicator */}
        {(step === "token" || step === "validating" || step === "error" || step === "branches" || step === "confirmed") && (
          <div className="flex items-center gap-2 px-6 py-2.5 bg-surface/50 border-b border-border-subtle shrink-0">
            ...
            {(step === "token" || step === "validating" || step === "error") && (
              <>
                ...
                <span className="text-xs text-muted">Personal Access Token</span>
              </>
            )}
            {/* Step 2: Branches */}
            {(step === "branches" || step === "confirmed") && (
          ...
          {step === "choose" && (
            <div className="space-y-3 py-1">
              ...
              <button
                ...
                className="w-full flex items-start gap-4 p-4 rounded-xl border-2 border-accent bg-accent/5 hover:bg-accent/10 transition-colors text-left group"
              >
                <div className="w-10 h-10 rounded-lg bg-accent/15 flex items-center justify-center shrink-0 group-hover:bg-accent/25 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold">Install GitHub App</span>
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-accent bg-accent/10 px-1.5 py-0.5 rounded">
                      Recommended
                    </span>
                  </div>
                  <p className="text-xs text-muted mt-0.5">
                    Org-level access. 1 click, no tokens needed. Works across all repos automatically.
                  </p>
                </div>
                <svg className="w-4 h-4 text-muted shrink-0 mt-0.5 group-hover:text-accent transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <button
                ...
                className="w-full flex items-start gap-4 p-4 rounded-xl border border-border bg-surface hover:bg-surface-hover transition-colors text-left group"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted mt-0.5">
                    Manual token. Per-user, per-repo. Use if your org doesn&apos;t allow app installs.
                  </p>
                </div>
                <svg className="w-4 h-4 text-muted shrink-0 mt-0.5 group-hover:text-foreground transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                </svg>
              </button>
            </div>
