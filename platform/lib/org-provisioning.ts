 ...
 * ════════════════════════════════════════════════════════════════════
 * Full workspace provisioning — called when a new org/workspace is
...
export async function provisionOrg(
  ...
  // ── 0. Ensure storage_config is set on the org row ────────────────
  try {
    ...
    const hasConfig = org?.storage_config && Object.keys(org.storage_config).length > 0;
  ...
  try {
    ...
    if (!existing) {
      const { error } = await service.from("org_settings").insert({
        organization_id: orgId,
        // All defaults from schema: learning enabled, calibration, consolidation, health, growth, realtime
      });
      if (error) throw error;
  ...
  try {
    ...
    if (!existing) {
      const isCore = orgId === CORE_ORG_ID;
      const { error } = await service.from("federation_config").insert({
        organization_id:         orgId,
        enabled:                 true,
        upstream_org_id:         isCore ? null : CORE_ORG_ID,
  ...
  try {
    for (const job of JOBS) {
      await service.from("scheduled_jobs").upsert(
        { organization_id: orgId, ...job, enabled: true },
  ...
  // If we have a customerId, pull all customer members and add to this workspace
...
export async function auditOrg(orgId: string): Promise<{
  orgId:   string;
  missing: string[];
  present: string[];
  healthy: boolean;
  ...
  return { orgId, missing, present, healthy: missing.length === 0 };
}
