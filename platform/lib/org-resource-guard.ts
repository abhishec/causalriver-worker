/**
 * Org Resource Guard — Multi-Tenancy Resource Limits (Week 7)
 * =============================================================
 *
 * Enforces per-org resource limits:
 *   - max_concurrent_connections (MCP SSE)
 *   - max active agent tasks
 *   - daily tool call budget
 *
 * Usage:
 *   const allowed = await checkOrgResources(supabase, orgId, 'mcp_connection');
...
// ============================================================================
// CHECK — Can this org consume another resource unit?
// ============================================================================

export async function checkOrgResources(
...
export async function incrementResource(
  ...
  // Upsert + increment via raw SQL RPC would be ideal, but for simplicity:
  const { data: usage } = await supabase
    .from("org_resource_usage")
    .select(field)
    .eq("organization_id", orgId)
