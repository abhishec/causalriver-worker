 ...
 * =====================
 * Writes cached health snapshots for each brain service layer to the service_health table.
 *
 * ADR-009 layer map:
 *   L26 = Process Intelligence (FSM/HITL state — always present, service_type='process-intelligence')
 *   L27 = SE-aaS              (optional service layer, service_type='se-aas')
 *   L28 = AaaS                (optional service layer, service_type='aas')
 *   L29 = PM-aaS              (optional service layer, service_type='pm-aas')
 *
...
export async function writeProcessIntelligenceHealth(
  ...
  try {
    ...
    // L26a: bpaas_process_instances current_state+status last 7d
    // L26b: agent_queue agent_type=bpaas task_type+status last 7d
    // L26c: State-level RL signals from cross_domain_signals (source_domain like process.%)
    // L26d: Top 3 templates by fitness_score (AlphaEvolve)
    ...
    const [instancesRow, bpaasJobsRow, stateSignalsRow, topTemplatesRow] = await Promise.all([
      ...
      // L26c: State-level RL signals from cross_domain_signals
      supabase
    ...
    // L26d: Top templates by AlphaEvolve fitness score
      ...
      .upsert(
        {
          ...
          service_type: "process-intelligence",
          summary: {
    ...
    logger.warn(
      "[ServiceHealthWriter] writeProcessIntelligenceHealth failed (non-fatal)",
      {
...
export async function writeSeaasHealth(
  ...
  try {
    ...
    // Mirror exact queries from brain-context.ts L27 (5 sub-queries):
    // L27a: agent_queue SE-aaS task types last 7d (uses task_type filter, same as brain-context)
    // L27b: scope_creep_alerts unresolved count (acknowledged=false)
    // L27c: engagement_health_latest bottom 3 (includes engagement_name)
    // L27d: engineer_health_snapshots flight_risk_score > 50 (desc order)
    // L27e: pod_match_history latest 3 (recommended_pod_name only)

    const [jobsRow, scopeCreepRow, engHealthRow, engineerRiskRow, podMatchRow] =
      await Promise.all([
        // L27a -- SE-aaS jobs: task_type IN [...] last 7d
        supabase
          .from("agent_queue")
        ...
        // L27c -- engagement_health_latest bottom 3 health scores
        supabase
        ...
        // L27e -- pod_match_history latest 3
        supabase
    ...
    // L27a: Job activity by domain (mirrors brain-context.ts assembly logic)
...
export async function writeAaasHealth(
  ...
  try {
    ...
    // L28a: se_aas_artifacts last 24h domain_type
    // L28b: agent_queue agent_type=aas last 7d status
...
// ── PM-aaS health writer (L29) ────────────────────────────────────────────────
// Queries PM-aaS specific agent_queue jobs (agent_type=pm-aas).
// Stamps service_type='pm-aas'. Separate from Process Intelligence (L26).

export async function writePmAaSHealth(
  supabase: SupabaseClient,
  orgId: string
): Promise<void> {
  try {
    ...
    // L29a: agent_queue agent_type=pm-aas last 7d, task_type+status
    const { data: pmaasJobsData } = await supabase
      .from("agent_queue")
      .select("task_type, status, created_at")
      .eq("organization_id", orgId)
    ...
    if (pmaasJobsData && pmaasJobsData.length > 0) {
      const statusCounts: Record<string, number> = {};
      const taskTypeCounts: Record<string, number> = {};
      for (const row of pmaasJobsData as Array<{ task_type: string; status: string }>) {
        const s = row.status ?? "unknown";
        statusCounts[s] = (statusCounts[s] ?? 0) + 1;
      ...
      const topTasks = Object.entries(taskTypeCounts)
        ...
        .join(", ");
      parts.push(`Agents(7d): ${succeeded} succeeded, ${failed} failed, ${running} running`);
      if (topTasks) parts.push(`Top tasks: ${topTasks}`);
    }

    const contextString =
      parts.length > 0
        ? `## PM-aaS Service Layer\n${parts.join(" | ")}`.slice(0, 300)
        : "";
      ...
      .upsert(
        {
          ...
          service_type: "pm-aas",
          summary: {
            jobCount7d: pmaasJobsData?.length ?? 0,
          },
          context_string: contextString,
          updated_at: new Date().toISOString(),
        },
    ...
    logger.warn("[ServiceHealthWriter] writePmAaSHealth failed (non-fatal)", {
      error: err instanceof Error ? err.message : String(err),
      orgId,
    });
  }
 ...
 *
 * Writes all 4 service health layers per org (ADR-009):
 *   L26 process-intelligence, L27 se-aas, L28 aas, L29 pm-aas
 */
export async function writeAllServiceHealth(
  ...
  try {
    ...
    // Write health for each org -- all 4 layers -- fire and forget per org
    for (const orgId of uniqueOrgIds) {
      writeProcessIntelligenceHealth(supabase, orgId).catch((e: unknown) =>
        logger.warn("[ServiceHealth] org write failed (process-intelligence)", { orgId, error: String(e) })
      );
      ...
      writePmAaSHealth(supabase, orgId).catch((e: unknown) =>
        logger.warn("[ServiceHealth] org write failed (pm-aas)", { orgId, error: String(e) })
      );
    }
