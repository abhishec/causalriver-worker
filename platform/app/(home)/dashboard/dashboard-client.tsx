...
/* ── Domain label map — keeps internal slugs out of the UI ─────────────── */
...
function toDomainLabel(slug: string | null | undefined): string {
  if (!slug) return "—";
...
export function DashboardClient() {
  ...
  // Workspace switcher dropdown (visible when multiple AI worker spaces exist)
      ...
      <nav className="border-b border-border-subtle bg-background/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {/* AI Worker Space switcher — only shown when multiple spaces exist under the customer */}
            {activeWorkspaces.length > 1 && currentWorkspace && (
              <div className="relative mr-2">
                <button
                  onClick={() => setWsDropdownOpen((prev) => !prev)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-foreground bg-surface hover:bg-surface-hover border border-border-subtle transition-colors"
                  aria-haspopup="listbox"
                  aria-expanded={wsDropdownOpen}
                >
                  <svg className="w-3.5 h-3.5 text-muted-foreground shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                  </svg>
                  <span className="max-w-[140px] truncate">{currentWorkspace.name}</span>
                  <svg className={`w-3 h-3 text-muted-foreground transition-transform ${wsDropdownOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {wsDropdownOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-1 z-50 min-w-[200px] rounded-lg bg-card border border-border shadow-lg py-1">
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (!isActive) switchWorkspace(m.organization_id);
                            }}
              ...
              <div className="mb-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="rounded-xl border border-border-subtle bg-surface p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">AI Workers</span>
                  </div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-2xl font-bold text-foreground tabular-nums">{aiWorkers.length}</span>
                ...
                <div className="rounded-xl border border-border-subtle bg-surface p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Agents Running</span>
                  </div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-2xl font-bold text-foreground tabular-nums">
                      {aiWorkers.reduce((sum, w) => sum + w.activeAgents, 0)}
                    </span>
                    <span className="text-[10px] text-muted-foreground">running</span>
                  </div>
                  <div className="mt-1 text-[10px] text-muted-foreground">
                    across {aiWorkers.length} worker{aiWorkers.length !== 1 ? "s" : ""}
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-border-subtle bg-surface p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Connectors</span>
                  </div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-2xl font-bold text-foreground tabular-nums">
                      {aiWorkers.reduce((sum, w) => sum + w.connectorCount, 0)}
                    </span>
                    <span className="text-[10px] text-muted-foreground">total</span>
                  </div>
                  <div className="mt-1 text-[10px] text-muted-foreground">
                    {[...new Set(aiWorkers.flatMap((w) => w.connectors))].length} unique types
                  </div>
                </div>

                <Link
                  href="/settings?tab=connections"
                  className="rounded-xl border border-dashed border-accent/30 bg-accent/5 p-4 hover:border-accent/60 hover:bg-accent/10 transition-all group flex flex-col justify-between"
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-accent">Manage</span>
                  </div>
                  <div className="text-sm font-medium text-accent">View All Workers</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5 group-hover:text-accent/70 transition-colors">
              ...
              <div className="mb-6 rounded-xl border border-accent/20 bg-gradient-to-r from-accent/5 via-surface to-emerald-500/5 overflow-hidden">
                <div className="px-5 py-4">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        ...
                        {brainIntelligence.isImproving && (
                          <span className="text-success ml-1">{"\u2022"} Actively improving</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div className="rounded-lg bg-background/60 border border-border-subtle px-3 py-2.5">
                      <div className="text-2xl font-bold text-success tabular-nums">{brainIntelligence.hoursSaved}h</div>
                  ...
                  {learningStats && learningStats.totalTasks > 0 && (
                    <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
                      ...
                      <div className="rounded-lg bg-background/40 border border-border-subtle px-3 py-2">
                        <div className="text-lg font-bold text-success tabular-nums">{Math.round(learningStats.successRate * 100)}%</div>
                      ...
                      <div className="rounded-lg bg-background/40 border border-border-subtle px-3 py-2">
                        <div className="text-lg font-bold text-foreground tabular-nums truncate">{toDomainLabel(learningStats.topDomain)}</div>
                          ...
                          {worker.brain && (
                            <>
                              <span className="flex items-center gap-1 font-medium text-accent">
                                ...
                                {worker.brain.trend === "improving" && <span className="text-success">{"\u2191"}</span>}
                              </span>
                              <span>{Math.min(100, Math.round(Number.isFinite(worker.brain.accuracy) ? worker.brain.accuracy * 100 : 0))}% accuracy</span>
                            </>
                          )}
                          {worker.activeAgents > 0 && (
                            <span className="text-success">{worker.activeAgents} agent{worker.activeAgents !== 1 ? "s" : ""} running</span>
                          )}
                        </div>
