...
function buildCSVExport(transactions: GLTransaction[], company: string): string {
  ...
  // Header meta
  sections.push([
    `# Brain OS AAS — Financial Output Package`,
    `# Company: ${company}`,
    `# Generated: ${now}`,
    `# Jurisdiction: SG | Currency: SGD | Standard: SFRS(I)`,
    `# Generated by Brain OS Accounting as a Service (AAS)`,
    '',
  ].join('\r\n'));
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filename = `BrainOS_AAS_${company.replace(/[^a-zA-Z0-9]/g, '_')}_${dateStr}.csv`;
