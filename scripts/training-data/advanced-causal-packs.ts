...
const aiAdoptionImpact: TrainingPack = {
  ...
  cascades: [
    {
      id: 'ai-productivity-flywheel',
      name: 'AI Productivity Flywheel',
      stages: [
        { signal: 'ai_tool_adoption', domain: 'engineering', description: 'Team adopts AI development tools (Copilot, AI testing, code review)' },
        { signal: 'developer_productivity_gain', domain: 'engineering', description: '55% faster task completion, 2x code review speed' },
        { signal: 'feature_shipping_velocity', domain: 'product', description: 'More features shipped per sprint, shorter cycle times' },
        { signal: 'customer_value_delivery', domain: 'cs', description: 'Faster feature delivery = higher customer satisfaction' },
        { signal: 'expansion_revenue_growth', domain: 'finance', description: 'Better product = more upsell/expansion' },
      ],
      totalLagDays: 120,
      confidence: 0.80,
    },
    {
      id: 'ai-trust-erosion-cascade',
      name: 'AI Trust Erosion Death Spiral',
      stages: [
        { signal: 'ai_quality_failure', domain: 'product', description: 'Hallucinations, wrong answers, unreliable AI outputs' },
        { signal: 'support_ticket_spike', domain: 'cs', description: '15%+ tickets become AI-related complaints' },
        { signal: 'user_trust_decline', domain: 'cs', description: 'Users stop trusting AI features, revert to manual' },
        { signal: 'feature_abandonment', domain: 'product', description: 'AI feature adoption drops, wasted R&D investment' },
        { signal: 'competitive_position_loss', domain: 'finance', description: 'Competitors with better AI gain market share' },
      ],
      totalLagDays: 90,
      confidence: 0.78,
    },
...
const maIntegrationPatterns: TrainingPack = {
  ...
  cascades: [
    {
      id: 'ma-value-destruction-cascade',
      name: 'M&A Value Destruction Spiral',
      stages: [
        { signal: 'acquisition_announcement', domain: 'finance', description: 'Deal announced, uncertainty begins' },
        { signal: 'cultural_friction', domain: 'hr', description: 'Conflicting processes, values, communication styles' },
        { signal: 'key_talent_departure', domain: 'hr', description: '20-40% of acquired key talent leaves within 12 months' },
        { signal: 'engineering_velocity_collapse', domain: 'engineering', description: 'Lost knowledge, broken teams, integration overhead' },
        { signal: 'customer_confusion_churn', domain: 'cs', description: 'Product roadmap uncertainty drives 15-25% elevated churn' },
        { signal: 'synergy_miss', domain: 'finance', description: 'Only 25% of acquirers achieve projected revenue synergies' },
      ],
      totalLagDays: 365,
      confidence: 0.75,
    },
...
const businessLifecyclePatterns: TrainingPack = {
  ...
  cascades: [
    {
      id: 'premature-scaling-death-spiral',
      name: 'Premature Scaling Death Spiral',
      stages: [
        { signal: 'rapid_hiring', domain: 'hr', description: 'Headcount grows 30-50% before PMF is validated' },
        { signal: 'burn_rate_acceleration', domain: 'finance', description: 'Monthly burn doubles/triples with new hires' },
        { signal: 'coordination_overhead', domain: 'engineering', description: 'More people = more meetings, less building' },
        { signal: 'product_direction_confusion', domain: 'product', description: 'Too many voices, no clear PMF signal' },
        { signal: 'runway_crisis', domain: 'finance', description: '6-9 months of runway triggers panic cuts' },
        { signal: 'layoffs_and_demoralization', domain: 'hr', description: 'Layoffs destroy culture, remaining team disengages' },
      ],
      totalLagDays: 270,
      confidence: 0.82,
    },
...
const techDisruptionCascade: TrainingPack = {
  ...
  cascades: [
    {
      id: 'christensen-disruption-cascade',
      name: 'Christensen Disruption Cascade',
      stages: [
        { signal: 'new_technology_emerges', domain: 'engineering', description: 'New tech appears: inferior on incumbent metrics, superior on new dimension (cheaper, simpler, more accessible)' },
        { signal: 'incumbent_dismissal', domain: 'product', description: 'Incumbents dismiss: "our customers dont want that" — focusing on high-end margin' },
        { signal: 'low_end_market_capture', domain: 'marketing', description: 'Disruptor captures non-consumers and low-end segments' },
        { signal: 'technology_improvement', domain: 'engineering', description: 'Disruptors technology improves along incumbent dimensions (quality, features)' },
        { signal: 'market_crossover', domain: 'product', description: 'Disruptor becomes "good enough" for mainstream, at lower price' },
        { signal: 'incumbent_revenue_collapse', domain: 'finance', description: 'Incumbents lose mainstream market. Revenue erodes "gradually then suddenly"' },
      ],
      totalLagDays: 1825,
      confidence: 0.78,
    },
...
const platformRiskDependency: TrainingPack = {
  ...
  cascades: [
    {
      id: 'platform-dependency-death-spiral',
      name: 'Platform Dependency Death Spiral',
      stages: [
        { signal: 'platform_success', domain: 'finance', description: 'Company grows rapidly on a single platform (App Store, Google, AWS Marketplace)' },
        { signal: 'increasing_dependency', domain: 'engineering', description: 'Engineering optimizes for platform-specific features, deepening lock-in' },
        { signal: 'platform_policy_change', domain: 'product', description: 'Platform changes rules: higher fees, new restrictions, API deprecation' },
        { signal: 'revenue_shock', domain: 'finance', description: 'Immediate 20-40% revenue impact from policy change' },
        { signal: 'scramble_to_diversify', domain: 'engineering', description: 'Panic migration to other platforms (costly, slow, quality suffers)' },
        { signal: 'competitive_disadvantage', domain: 'finance', description: 'Competitors who diversified early capture displaced customers' },
      ],
      totalLagDays: 365,
      confidence: 0.80,
    },
...
const economicCycleImpact: TrainingPack = {
  ...
  cascades: [
    {
      id: 'recession-saas-cascade',
      name: 'Recession SaaS Impact Cascade',
      stages: [
        { signal: 'economic_contraction_signal', domain: 'finance', description: 'GDP decline, interest rate hikes, unemployment rising' },
        { signal: 'enterprise_budget_freeze', domain: 'cs', description: 'Customer finance teams freeze discretionary spend' },
        { signal: 'vendor_consolidation', domain: 'cs', description: 'Companies cut vendors from 50+ to 30. Nice-to-have tools go first.' },
        { signal: 'sales_cycle_elongation', domain: 'marketing', description: 'Sales cycles extend 30-50%. More approvals, more scrutiny.' },
        { signal: 'churn_acceleration', domain: 'finance', description: 'Net revenue retention drops 5-15 points below normal' },
        { signal: 'valuation_compression', domain: 'finance', description: 'Revenue multiples compress 40-70% from peak. Growth penalized, efficiency rewarded.' },
      ],
      totalLagDays: 180,
      confidence: 0.85,
    },
    {
      id: 'recovery-growth-cascade',
      name: 'Recovery Growth Cascade',
      stages: [
        { signal: 'economic_recovery_signal', domain: 'finance', description: 'GDP growth resumes, interest rates stabilize or decline' },
        { signal: 'budget_thaw', domain: 'cs', description: 'Enterprise budgets unfreeze, new projects get approved' },
        { signal: 'pent_up_demand_release', domain: 'marketing', description: 'Delayed purchases convert. Pipeline velocity accelerates 2-3x.' },
        { signal: 'growth_acceleration', domain: 'finance', description: 'Revenue growth rebounds above pre-recession levels' },
        { signal: 'valuation_expansion', domain: 'finance', description: 'Revenue multiples expand. Growth re-prioritized by investors.' },
      ],
      totalLagDays: 270,
      confidence: 0.80,
    },
