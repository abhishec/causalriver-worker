    ...
    const packs: TrainingPack[] = [
      {
        ...
        businessRules: [
          ...
          { id: 'in-gst-itc-offset-order', condition: 'jurisdiction = in AND has_itc', action: 'ITC offset order (mandatory, cannot choose): Step 1: IGST credit → offset IGST output first, then CGST, then SGST. Step 2: CGST credit → offset CGST output first, then IGST (NOT SGST). Step 3: SGST credit → offset SGST output first, then IGST (NOT CGST). Cross-offset of CGST to SGST or SGST to CGST is NOT permitted.', confidence: 1.0, source: 'cgst_rule_88a' },
          { id: 'in-gst-rcm-import-services', condition: 'jurisdiction = in AND payment_to = non_resident OR associated_enterprise_abroad', action: 'Import of services from non-resident: Reverse Charge Mechanism (RCM) applies u/s 5(3) IGST Act. India entity self-assesses GST and pays via cash ledger (cannot use ITC to pay RCM). Rate 18% on management fees/royalties. If used for zero-rated exports, RCM paid = Input ITC claimable in next period.', confidence: 1.0, source: 'igst_section_5_3_rcm' },
        ],
        cascades: [],
      ...
      } as unknown as TrainingPack,
      {
        ...
        businessRules: [
          ...
          { id: 'in-tp-form-3ceb', condition: 'entity = tookitaki_in AND total_international_transactions > 10000000', action: 'Transfer Pricing: If total international transactions with Tookitaki Holding (SG) exceed ₹1 Cr in a year, mandatory: (1) TP study documenting arm\'s length price, (2) Form 3CEB (CA certification), (3) File with ITR. Risk: CBDT can add back if TP adjustment made → higher tax + 2-14% penalty. Use TNMM as primary method for services.', confidence: 1.0, source: 'cbdt_section_92e_form_3ceb' },
        ],
        cascades: [],
