...
export class AASSGDeepTrainerAgent extends BaseTrainingAgent {
    ...
    const packs: TrainingPack[] = [
      {
        ...
        businessRules: [
          ...
          { id: 'sg-gst-9pct', condition: 'sg_transaction_date >= 2024-01-01', action: 'Apply GST 9%. Xero taxRateName = "GST 9%". Output tax = amount × 0.09 (exclusive) or amount × 9/109 (inclusive). Current rate.', confidence: 1.0, source: 'iras_gst_announcement_nov2023' },
          { id: 'sg-gst-mixed-gl', condition: 'xero_gl_spans_multiple_years', action: 'ALWAYS read taxRateName from each GL row — do not assume a single GST rate. A GL from 2020-2025 will contain "GST 7%", "GST 8%", "GST 9%" entries. Use row-level rate for each transaction.', confidence: 1.0, source: 'parse_xero_gl_ts_observation' },
        ],
        cascades: [],
...

      {
        ...
        businessRules: [
          ...
          { id: 'sg-sdl', condition: 'sg_employee_any', action: 'SDL = 0.25% of monthly wages. Min $2, max $11.25 per employee. Debit SDL Expense, Credit SDL Payable. Remit with CPF by 14th.', confidence: 1.0, source: 'skillsfuture_sdl_guide' },
          { id: 'sg-sfrs16', condition: 'lease_term > 12_months AND not_short_term_lease', action: 'Recognise ROU Asset + Lease Liability at commencement (PV of lease payments). Amortise ROU over lease term. Unwind interest on Lease Liability to P&L. Prior operating lease expense replaced by depreciation + interest.', confidence: 1.0, source: 'sfrs_i_16_leases' },
          { id: 'sg-wht-overseas', condition: 'payment_to_non_resident AND (management_fee OR royalty OR interest OR service_fee)', action: 'Withholding tax applies. Rate: management fees 17%, royalties 10%, interest 15% (may be reduced by tax treaty). Debit WHT Expense (if borne by payer), Credit WHT Payable. File Form IR37 with IRAS within 1 month of payment.', confidence: 0.95, source: 'iras_withholding_tax_guide' },
        ],
        cascades: [{ trigger: 'cpf_payable_overdue', effects: ['iras_late_payment_penalty', 'cpf_board_prosecution_risk'], probability: 0.85 }],
  ...
  async train(signals: ConnectorSignal[], packs: TrainingPack[]): Promise<TrainResult> {
    ...
    try { const trainer = createBrainTrainer(); const r = await trainer.trainBatch(this.supabase, this.organizationId, packs); result.packsProcessed = packs.length; result.discoveries = r?.casesLoaded ?? 0; } catch (err) { this.errors.push(`Brain trainer: ${err}`); }
