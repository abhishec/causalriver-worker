    ...
    const packs: TrainingPack[] = [
      {
        ...
        businessRules: [
          { title: 'Philippines VAT 12% — Input/Output with Net Filing', entityType: 'transaction', when: { logic: 'AND', conditions: [{ field: 'jurisdiction', operator: 'equals', value: 'ph' }, { field: 'is_vat_registered', operator: 'equals', value: true }, { field: 'is_taxable_supply', operator: 'equals', value: true }] }, then: [{ type: 'set_flag', params: { vat_rate: 0.12, base: 'gross_selling_price', input_vat_claimable: true, formula: 'Net VAT = Output VAT - Input VAT', filing: 'Form 2550M monthly by 20th' } }], naturalLanguage: 'Philippines VAT is 12% of the gross selling price. Output VAT is collected on sales. Input VAT on qualifying purchases is claimable. Net VAT payable = Output VAT - Input VAT. File Form 2550M monthly by the 20th. (BIR NIRC Sec 106/108)' },
          { title: 'Philippines Expanded Withholding Tax on Professional Fees', entityType: 'transaction', when: { logic: 'AND', conditions: [{ field: 'jurisdiction', operator: 'equals', value: 'ph' }, { field: 'payment_to_type', operator: 'equals', value: 'professional_individual' }] }, then: [{ type: 'set_flag', params: { ewt_rate_under_3m: 0.10, ewt_rate_over_3m: 0.15, threshold_php: 3000000, payor_obligation: 'withhold and remit', certificate: 'BIR Form 2307 to payee' } }], naturalLanguage: 'Philippines EWT on professional fees: 10% if payee income ≤₱3M/year, 15% if >₱3M. The payor (your company) withholds the EWT from the payment and remits to BIR. Issue BIR Form 2307 as proof of withholding to the professional. (BIR RR 11-2018)' },
          { title: 'Philippines 13th Month Pay — Mandatory Benefit', entityType: 'payroll_transaction', when: { logic: 'AND', conditions: [{ field: 'jurisdiction', operator: 'equals', value: 'ph' }, { field: 'employee_type', operator: 'equals', value: 'rank_and_file' }] }, then: [{ type: 'set_flag', params: { formula: 'total_basic_salary_earned_in_year / 12', accrual: 'monthly_1_12th', payment_deadline: 'December 24', tax_exempt_threshold_php: 90000, note: 'Combined with other benefits under TRAIN Law' } }], naturalLanguage: 'Mandatory 13th Month Pay = total basic salary earned in the calendar year ÷ 12. Accrue 1/12 per month (DR 13th Month Expense, CR 13th Month Payable). Pay by December 24. Tax-exempt up to ₱90,000 combined with other benefits under the TRAIN Law. (PD 851 / DOLE)' },
          { title: 'Philippines SSS, PhilHealth and Pag-IBIG Contributions', entityType: 'payroll_transaction', when: { logic: 'AND', conditions: [{ field: 'jurisdiction', operator: 'equals', value: 'ph' }, { field: 'is_ph_employee', operator: 'equals', value: true }] }, then: [{ type: 'set_flag', params: { employer_sss_rate: 0.095, employer_philhealth_rate: 0.05, employer_pagibig_rate: 0.02, pagibig_max_php: 200, remittance: 'monthly' } }], naturalLanguage: 'Philippines mandatory employer contributions: SSS 9.5%, PhilHealth 5%, Pag-IBIG 2% (max ₱200/month). All three have matching employee deductions. Total employer burden is ~17% of basic salary on top of gross pay. Remit monthly. (SSS / PhilHealth / HDMF)' },
        ],
        cascades: [],
