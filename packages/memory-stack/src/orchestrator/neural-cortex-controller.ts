...
import type { CognitiveStackInstance, CognitiveCycleInput, CognitiveCycleResult, StreamingCycleHandle } from './cognitive-stack';
...
export interface NeuralCortexInstance {
  ...
  /**
   * Alias for runCycle() — used by the brain cycle API route.
   * Keeps the public API consistent (route calls runManagedCycle, controller
   * internally decides which layers to schedule).
   */
  runManagedCycle(input: CognitiveCycleInput): Promise<ManagedCycleResult>;

  /**
   * STREAMING ENTRY POINT — process signals in 500-signal batches.
   *
   * The controller delegates batch dispatch to the cognitive stack's
   * beginStreamingCycle(), which accumulates state across batches and
   * runs blocking layers (L14 DAG, L15 narrative) once in finalize().
   *
   * Usage from brain cycle API:
   *   const handle = controller.beginStreamingCycle(inputWithoutSignals);
   *   for await (const batch of signalStream) {
   *     handle.processBatch(batch);
   *   }
   *   result = await handle.finalize();
   *
   * Memory profile: never more than 500 × 450B = ~225KB of signals in RAM.
   */
...
export function createNeuralCortexController(config: NeuralCortexConfig): NeuralCortexInstance {
  ...
  // Now passes supabase + organizationId for RL state persistence across restarts.
  ...
  // compute budgets, and exploration rates on every restart.
  // The brain loads from brain_rl_state table (created by Task 1.5).
  (async function _warmStartRL() {
    if (!_rl) return;
    try {
      const { data: rlState } = await supabase
        .from('brain_rl_state')
        .select('layer_id, scheduling_multiplier, compute_budget_multiplier, exploration_rate, cumulative_reward, output_threshold')
        .eq('organization_id', organizationId)
        .order('updated_at', { ascending: false });

      if (rlState?.length) {
        for (const row of rlState) {
          const state = _rl.getLayerState(row.layer_id);
          if (state) {
            state.schedulingMultiplier = row.scheduling_multiplier ?? state.schedulingMultiplier;
            state.computeBudgetMultiplier = row.compute_budget_multiplier ?? state.computeBudgetMultiplier;
            state.explorationRate = row.exploration_rate ?? state.explorationRate;
            state.cumulativeReward = row.cumulative_reward ?? state.cumulativeReward;
            state.outputThreshold = row.output_threshold ?? state.outputThreshold;
          }
        }
        console.log(`[NeuralCortex] Warm-started RL: loaded state for ${rlState.length} layers`);
      }
    } catch (err: any) {
  ...
  return {
    async runCycle(input: CognitiveCycleInput): Promise<ManagedCycleResult> {
      ...
      try {
        if (needsDeepLayers && (_mode === 'awake_full' || _mode === 'sleeping' || _mode === 'dreaming')) {
          ...
          // Wire L29 → Loop 4: persist intervention observation windows
          _persistInterventionWindows(fullResult.deep);
      ...
      // 2b. PERSIST AUTO-GENERATED PREDICTIONS (The Missing Wire for Loop 1)
      // Cognitive stack now harvests predictions from L5, L8, L10, L14.
      // We persist them to prediction_records so the closed-loop engine can verify them.
      // Without this: Loop 1 has nothing to verify → brain never learns.
      const cycleResult = brainResult ?? fullResult?.brain;
      if (cycleResult?.autoGeneratedPredictions?.length) {
        try {
          const predictionRows = cycleResult.autoGeneratedPredictions.map(p => ({
            id: p.id,
            organization_id: organizationId,
            domain: p.domain,
            entity_type: p.entityType,
            entity_id: p.entityId,
            predicted_outcome: p.predictedDirection,
            predicted_value: p.predictedValue ?? null,
            confidence: p.confidence,
            was_correct: null, // Pending verification by Loop 1
            ...
            metadata: {
              ...
              outcome_deadline: new Date(Date.now() + p.outcomeWindowMs).toISOString(),
              evidence: p.evidence,
              method: p.method,
            },
          }));

          // Batch insert (fire-and-forget, non-blocking)
          supabase.from('prediction_records').insert(predictionRows).then(({ error }) => {
            if (error) console.warn('[NeuralCortex] Prediction persistence non-fatal:', error.message);
          });
        } catch (err: any) {
          console.warn('[NeuralCortex] Prediction persistence error:', err?.message || err);
        }
      }
      ...
      // 4b. Run closed-loop learning (every 5th cycle during normal operation)
      // Tightened from 10 → 5 to close loops faster. Each cycle:
      //   Loop 1: Verify pending predictions against actual signals
      //   Loop 2: Update causal weights via Bayesian posteriors
      //   Loop 3: Process queued user feedback
      //   Loop 4: Check intervention observation windows
      //   Loop 5: Auto-generate training packs if enough new data
      //   Loop 6: Track agent outcomes for RL credit assignment
      let learning: LearningCycleResult | null = null;
      if (_closedLoop && _cycleCount % 5 === 0) {
        try {
    ...
    async runAgentCycle(input: CognitiveCycleInput, agentId: string): Promise<FullCycleResult> {
      ...
      _updateBrainLayerStats(fullResult.brain);
      _updateDeepLayerStats(fullResult.deep);
      // Wire L29 → Loop 4 for agent cycles too
      _persistInterventionWindows(fullResult.deep);

      // Persist auto-generated predictions from agent cycle
      if (fullResult.brain?.autoGeneratedPredictions?.length) {
        try {
          const predictionRows = fullResult.brain.autoGeneratedPredictions.map(p => ({
            id: p.id,
            organization_id: organizationId,
            domain: p.domain,
            entity_type: p.entityType,
          ...
          supabase.from('prediction_records').insert(predictionRows).then(({ error }) => {
            if (error) console.warn('[NeuralCortex] Agent prediction persistence non-fatal:', error.message);
          });
        } catch (err: any) {
          console.warn('[NeuralCortex] Agent prediction persistence error:', err?.message || err);
        }
      }
    ...
    // ── runManagedCycle: alias for runCycle (consistent API) ─────────────
    async runManagedCycle(input: CognitiveCycleInput): Promise<ManagedCycleResult> {
      return this.runCycle(input);
    },

    // ── beginStreamingCycle: dispatch 500-signal batches to cognitiveStack ─
    beginStreamingCycle(input: Omit<CognitiveCycleInput, 'signals'>): StreamingCycleHandle {
      // The cognitive stack owns the streaming implementation.
      // The controller wraps it so callers only need the NeuralCortexInstance.
      const handle = cognitiveStack.beginStreamingCycle(input);

      // Wrap finalize() to also run RL feedback + persist predictions,
      // mirroring what runCycle() does after a regular managed cycle.
      const originalFinalize = handle.finalize.bind(handle);

      const wrappedFinalize = (): CognitiveCycleResult => {
        const brainResult = originalFinalize();

        // Persist auto-generated predictions (non-blocking, fire-and-forget)
        if (brainResult?.autoGeneratedPredictions?.length && supabase) {
          try {
            const predictionRows = brainResult.autoGeneratedPredictions.map(p => ({
              id: p.id,
              organization_id: organizationId,
              domain: p.domain,
              entity_type: p.entityType,
              ...
              metadata: {
                ...
                evidence: p.evidence,
                method: p.method,
                source: 'streaming_cycle',
              },
            }));
            supabase.from('prediction_records').insert(predictionRows).then(({ error }: { error: any }) => {
              if (error) console.warn('[NeuralCortex] Streaming prediction persist non-fatal:', error.message);
            });
          } catch (err: any) {
            console.warn('[NeuralCortex] Streaming prediction persist error:', err?.message || err);
          }
        }

        // RL feedback — update layer health from streaming cycle outcome
        if (_rl && brainResult) {
          const layerMetrics = new Map<number, Record<string, number>>();
          for (const [id, layer] of _layers) {
            if (Object.keys(layer.lastMetrics).length > 0) {
      ...
      return {
        processBatch: handle.processBatch.bind(handle),
        finalize: wrappedFinalize,
        getState: handle.getState.bind(handle),
      };
    },
    ...
    async runSleepCycle(): Promise<SleepCycleResult> {
      ...
      try {
        ...
        // Wire L29 → Loop 4 during sleep too
        _persistInterventionWindows(deepResult);
  ...
  /**
   * Wire L29 Intervention Recommender → Loop 4 Intervention Outcome Tracking.
   *
   * When L29 generates recommendations, we create observation windows
   * so Loop 4 can later check if the interventions actually worked.
   * Without this wire: L29 recommendations vanish, the brain never knows
   * if its advice was good, and rule confidence stays frozen.
   */
  function _persistInterventionWindows(deepResult: DeepCycleResult): void {
    if (!_closedLoop || !deepResult.interventions.recommended.length) return;

    for (const intervention of deepResult.interventions.recommended) {
      // Find current metric value for baseline
      const baselineValue = intervention.expectedImpact || 0;

      _closedLoop.recordInterventionRecommended({
        ruleId: `L29_${intervention.sourceDomain}_${intervention.targetDomain}`,
        entityType: intervention.targetDomain,
        entityId: `${intervention.sourceDomain}→${intervention.targetDomain}`,
        metricName: intervention.action,
        baselineValue,
        targetValue: baselineValue * (1 + intervention.expectedImpact),
        windowDays: 14, // 2 weeks to observe intervention outcome
      }).catch((err: any) => {
        console.warn('[NeuralCortex] Intervention window creation non-fatal:', err?.message || err);
      });
    }
  }
