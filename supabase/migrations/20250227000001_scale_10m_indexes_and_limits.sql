...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_signals_org_domain_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_signals_org_time_value
  ON cross_domain_signals (organization_id, created_at DESC, signal_value DESC);

-- For entity-level signal lookups with time range
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_signals_org_entity_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_causal_org_sig_effect
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_causal_org_evidence
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_predictions_org_entity_time
  ON prediction_records (organization_id, entity_type, entity_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_predictions_org_verified
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_memory_org_type_importance
  ON ai_memory (organization_id, memory_type, importance DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_memory_org_type_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_temporal_org_relevance
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_rules_org_active_confidence
  ON brain_grammar_rules (organization_id, is_active, confidence DESC)
  WHERE is_active = true;


-- ─── 7. VECTOR INDEX UPGRADE: IVFFlat 100 → 1000 LISTS ────────────────────
-- At 10M embeddings with 100 lists: 100K embeddings per bucket (slow scan)
-- At 10M embeddings with 1000 lists: 10K embeddings per bucket (10x faster)
--
-- Strategy: drop old index, create new with 1000 lists
-- Note: This is safe because CONCURRENTLY doesn't hold locks
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_execution_org_time
  ON brain_execution_log (organization_id, created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_activity_org_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_events_org_domain_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_consolidation_org_status_time
